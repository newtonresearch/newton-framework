...
CUSoundChannel::~CUSoundChannel()
{
	...
	SoundNode * node, * next;
	for (node = fRecycledNodes; node != NULL; node = next)
	{
		next = node->next;
		delete node;
	}
...
CUSoundChannel::open(int inInputDevice, int inOutputDevice)
{
	...
	XTRY
	{
		...
		XFAIL(err = sendImmediate(comprSelector, 0, f18, &reply, sizeof(reply)))
		XFAILIF(err = reply.fError, close();)	// sic -- shouldn’t we close if any error?
		f1C = reply.fChannel;
	}
	XENDTRY;
	return err;
...
CUSoundChannel::close(void)
{
	...
	XTRY
	{
		...
		CUSoundReply	reply;
		if (f1C != 0)
			err1 = sendImmediate(kSndCloseChannel, 0, f1C, &reply, sizeof(reply));
		if (f18 != 0)
			err2 = sendImmediate(kSndCloseChannel, 0, f18, &reply, sizeof(reply));
...
CUSoundChannel::schedule(SoundBlock * inSound, CUSoundCallback * inCallback)
{
	...
	XTRY
	{
		if (fVolume == -INFINITY && (fState & 0x80) != 0)
			fVolume = getVolume();
		long	r8 = (inSound->codec == NULL) ? f18 : f1C;
		...
		XFAILNOT(r8, err = kSndErrChannelIsClosed;)

		XFAIL(err = makeNode(&theNode))
		theNode->cb = inCallback;
		theNode->request.fChannel = r8;
...
CUSoundChannel::cancel(ULong inRefCon)
{
	...
	XTRY
	{
		XFAILNOT(f18, err = kSndErrChannelIsClosed;)

		SoundNode * node = findRefCon(inRefCon);
		XFAILNOT(node, err = kSndErrNoSamples;)

		CUSoundReply	reply;
		err = sendImmediate(kSndCancelNode, f18, node->id, &reply, sizeof(reply));
		if (err == noErr)
			err = reply.fError;
	}
	XENDTRY;
...
CUSoundChannel::start(bool inAsync)
{
	...
	XTRY
	{
		XFAILNOT(f1C, err = kSndErrChannelIsClosed;)

		CUSoundReply	reply;
		err = sendImmediate(inAsync ? kSndStartChannel : kSndStartChannelSync, f1C, 0, &reply, sizeof(reply));
		if (err == noErr)
			err = reply.fError;
		XFAIL(err)
		fState &= ~kSoundPaused;
...
CUSoundChannel::pause(SoundBlock * outSound, long * outIndex)
{
	...
	XTRY
	{
		...
		else
		{
			...
			err = sendImmediate(kSndPauseChannel, f1C, 0, &reply, sizeof(reply));
			if (err == noErr)
				err = reply.fError;
			XFAIL(err)
...
CUSoundChannel::stop(SoundBlock * outSound, long * outIndex)
{
	...
	XTRY
	{
		...
		{
			CUSoundNodeReply	reply;
			err = sendImmediate(kSndStopChannel, f1C, 0, &reply, sizeof(reply));
			if (err == noErr)
				err = reply.fError;
			XFAIL(err)
...
{
	SoundNode * node, * next;
	for (node = fActiveNodes; node != NULL; node = next)
	{
...
CUSoundChannel::setOutputDevice(int inDevice)
{
	...
	{
		CUSoundReply	reply;
		sendImmediate(kSndSetOutputDevice, f18, inDevice, &reply, sizeof(reply));
	}
}
...
CUSoundChannel::setVolume(float inDecibels)
{
	...
	{
		CUSoundReply	reply;
		if (sendImmediate(kSndSetVolume, 0, inDecibels, &reply, sizeof(reply)) == noErr)
...
CUSoundChannel::setInputGain(int inGain)
{
	...
	CUSoundReply	reply;
	sendImmediate(kSndSetInputGain, f18, inGain, &reply, sizeof(reply));
}
...
{
	SoundNode * node, * prev = NULL;

	for (node = fActiveNodes; node != NULL; prev = node, node = node->next)
	{
		// look for node
...
CUSoundChannel::makeNode(SoundNode ** outNode)
{
	...
	else
	{
		XTRY
		{
			...
			XFAILNOT(theNode, err = MemError();)
#if !defined(forFramework)
			XFAIL(err = theNode->msg.init(YES))
...
CFrameSoundChannel::CFrameSoundChannel()
	:	fCodec(NULL), fIsCodecReady(NO), fCallback(this)
{ }
...
CFrameSoundChannel::convert(RefArg inSound, SoundBlock * outParms)
{
	...
	XTRY
	{
		...
		if (IsReal(item))
			...
			outParms->sampleRate = *(Fixed *)BinaryData(item) / (Fixed)0x00010000L;	// FixedToFloat
		else
